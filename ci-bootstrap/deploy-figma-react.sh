...
echo "🎨 FIGMA REACT: Deploying React code with Figma design..."
echo "========================================================"
...
# Check if we have the Figma React template
if [ -d "improved-template" ]; then
    echo "✅ Found Figma React template, attempting build..."
...
EOF
    ...
    echo "🔨 Building React application with Figma design..."
    if npm run build 2>/dev/null; then
        ...
        if [ -d "dist" ]; then
            ...
            cd ..
            echo "🎉 Figma React app deployed successfully!"
            exit 0
        elif [ -d "build" ]; then
    ...
    echo "⚠️ Figma React template not found, creating enhanced HTML version..."
fi

# Create enhanced HTML based on Figma design as fallback
echo "🎨 Creating enhanced HTML based on Figma design..."
...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Bebas+Neue:wght@400&family=Montserrat:wght@300;400;600;700&family=Roboto:wght@300;400;500;700&display=swap" rel="stylesheet">
    <style>
        * {
            margin: 0;
            padding: 0;
...
echo "✅ Enhanced HTML based on Figma design created successfully!"
echo "🎉 Figma design deployed!"
